...
description: .specsの実装計画に沿って実装し、各タスク完了後にClaude Code CLIで自動コードレビュー。Claude Codeでレビューしたい環境向け。
disable-model-invocation: true
argument-hint: "[番号]"
allowed-tools: Bash(claude *), Bash(mkdir *), Bash(rm .specs/*/PLANNING), Bash(rm .specs/.guard/*)
---

# Spec Implement (Claude Code版)

番号指定で `.specs/{nnn}-{feature-name}/` の実装計画に沿って実装を進めるスキル。
各タスクの実装後に **Claude Code CLI** でコードレビューを行い、品質を担保する。
...
5. タスクを順番に実装（□のタスクを処理）
   ↓
6. 各タスク完了時に Claude Code レビュー → 修正ループ
   ↓
...
## Step 5: Claude Code レビュー（各タスク完了後）

タスクの実装が完了したら、Claude Code CLI でコードレビューを実行する。
...
- 各タスク完了ごとに Claude Code レビューを実施する
