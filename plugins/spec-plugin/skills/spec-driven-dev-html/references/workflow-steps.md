...
exploration-report と hearing-notes を統合し、ユースケースと要件・制約を確定した **requirements.html** を**オーケストレーター自身**が生成する（サブエージェントには委譲しない — ユーザーと対話して確定するため）。requirements は hook のパース対象だが、hook が HTML 構造を解釈するため他の生成物と同様に **`.html`** で出力する。

### 3.5-1. requirements.html 起案

テンプレート `assets/templates/requirements.html` を Read して埋める:
...
「コードベースを調べても解決できず、ユーザーにしか決められない分岐」を **未解決の確認事項** に未チェックのチェックボックス（`<li><span class="checkbox"></span>...</li>`）で列挙する。以下を抽出源とする（exploration-report のスキャン）:
...
判定軸はタスクの規模（行数等の外形）ではなく「**コードベースを見ても解決できない＝ユーザーに聞くしかない分岐が残ったか**」。コードから一意に決まる事項は未チェック項目にせず、要件・制約として確定させる。

### 3.5-3. 未解決の有無で分岐

- **未チェック 0 件**: 未解決の確認事項のリストを削除して「なし」と記載し、Step 4 へ進む
- **未チェック 1 件以上**: 3.5-4 へ進む

### 3.5-4. AskUserQuestion で解消

未チェック項目を重要度順に並べ、上位 4 件（AskUserQuestion 上限）を 1 ターンで一括聴取する。
...
回答を requirements.html に反映する:

- 確定した分岐は該当項目の `<span class="checkbox">` を `<span class="checkbox done">` に変え、決定内容を要件・制約セクションにも反映する
- ユーザーが Other で自由記述した場合はその内容も記録する
- すべて解消したら、未解決の確認事項を「なし」にできる

> **重要**: 未チェックの項目（`<span class="checkbox">`）が残ったまま Step 4 で implementation-plan を書こうとすると `guard-requirements.sh` がブロックする。AutoMode でも解消をスキップしてはならない。
