...
description: .plugin-workspace/.specsの実装計画に沿って実装し、全タスク完了後にGitHub Copilot CLIで一括コードレビュー。Codexが使えない環境向け。
disable-model-invocation: true
argument-hint: "[番号]"
allowed-tools: Bash(copilot *), Bash(mkdir *), Bash(rm .plugin-workspace/.specs/*/PLANNING), Bash(rm .plugin-workspace/.specs/.guard/*)
---

# Spec Implement (Copilot版)

番号指定で `.plugin-workspace/.specs/{nnn}-{feature-name}/` の実装計画に沿って実装を進めるスキル。
全タスクの実装完了後に **GitHub Copilot CLI** で一括コードレビューを行い、品質を担保する。
