...
冗長に見えるコードにspec上の根拠があるかを判定する。**このプラグインの核心次元。**

以下のコードに遭遇したとき、インテントマップを参照して PROTECTED 判定する：

- **冗長に見えるエラーハンドリング** → hearing-notes でエッジケース処理が要求されているか？
- **分離されたループ** → implementation-plan で可読性のために分離が選択されているか？
...
**PROTECTED の出力フォーマット**:
```
- **表面的な印象**: {ナイーブなレビューアーならこう言う}
- **仕様による正当化**: {spec文書のどこがこのコードを正当化するか、具体的に引用}
- **結論**: 現状維持を推奨
```
...
- 意図的複雑性の保護（次元4）は最も重要な次元 — 迷ったら PROTECTED 寄りに判定する
